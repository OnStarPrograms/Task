...
function randomXY()
{
  let myWidth = window.innerWidth/200;
  let myHeight = window.innerHeight/200;
  let height = 0;
  for (let i = 0; i < count; i++)
  {
    Coord.push(Math.floor(i % (myWidth-200))*200);//randomx
    if (i % myWidth-1 < 0){
      height += 1;
    }
    Coord.push((Math.floor(height % myHeight)-1)*200);//randomY
...
{ //TimeStamp
  ...
  //if (mouseDown)
    //{
      //if (Coord[i]-30 < posX && posX <  Coord[i]-30+200 && Coord[i+1]-15 < posY && posY <  Coord[i+1]-15+200){
        ...
        //if (prevcoord[0] == -1){
          ...
          //prevcoord[1] = posY
        //}
        //Coord[i] += posX-prevcoord[0];
        //Coord[i+1] += posY-prevcoord[1];
        //prevcoord[0] = posX;
        //prevcoord[1] = posY;
    ...
    if (clicked)
    {
      if (Coord[i] < posX && posX <  Coord[i]-30+200 && Coord[i+1] < posY && posY <  Coord[i+1]+200){
        alert(MNText);
      }
    }
...
function getInputedValues()
{
  ...
  setTimeout(() => {
     document.getElementById('_former').innerHTML = '<button onclick="loadform()">New Task</button> <form\
      action="https://docs.google.com/forms/u/0/d/e/1FAIpQLSehw2OE7C_HlXgRb4kcx8AbcmcXB5SdL_WSO__CU-htnmCzSA/formResponse"\
      target="hiddenFrame" id = "form" style = "float:right;">\
      <input type="text" name="entry.498945756" value="Name" id="name">\
      <input type="text" name="entry.1254765034" value="Text" id="text" style="display:none;">\
      <input type="text" name="entry.762508820" value="Finished" id="notenum" style="display:none;">\
      <input type="submit" value="Finished" onClick="getInputedValues()">\
    </form>';
  }, 100);
